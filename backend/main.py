...
from ai_engine.chat import chat_with_divu, chat_stream_divu
from ai_engine.rag import process_document, query_rag
...
try:
    firebase_admin.initialize_app(options={'projectId': 'divu-ai'})
except ValueError:
    pass
...
app = FastAPI(
    title="Divu AI Assistant API",
    description="Optimized Backend API for the Divu AI Assistant with rate limiting, caching, and robust error handling.",
    version="1.2.0"
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "http://127.0.0.1:5173", "https://divu-ai.web.app"],
...
@app.get("/")
def read_root():
    return {"message": "Welcome to Divu AI Assistant API. The backend is running and optimized!"}
...
async def chat_endpoint(request: Request, req: ChatRequest, user=Depends(verify_token)):
    ...
    # 4. Process Chat 
    if req.stream:
        return StreamingResponse(chat_stream_divu(user_input, req.history), media_type="text/event-stream")
    else:
        result = await chat_with_divu(user_input, session_id, req.history)
...
async def speech_to_speech_endpoint(request: Request, audio: UploadFile = File(...), session_id: str = Form(None), user=Depends(verify_token)):
    try:
        ...
        chat_result = await chat_with_divu(user_text, session_id, [])
...
async def gesture_chat_endpoint(request: Request, image: UploadFile = File(...), session_id: str = Form(None), user=Depends(verify_token)):
    try:
        ...
        chat_result = await chat_with_divu(prompt, session_id, [])
        ai_text = chat_result["response"]
