...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/divu';
...
app.use(helmet());
app.use(cors({
    origin: ['http://localhost:5173', 'http://127.0.0.1:5173', 'https://divu-ai.web.app'],
    credentials: true,
...
// Root Route
app.get('/', (req, res) => {
    res.json({ message: "Welcome to Divu AI Assistant API (Node.js & MongoDB version)." });
});

// ─── Start HTTP server IMMEDIATELY (before DB connects) ───────────────────────
// This prevents the frontend from getting ECONNREFUSED even during startup.
app.listen(PORT, () => {
    console.log(`Divu backend listening on http://localhost:${PORT}`);
    console.log('Connecting to MongoDB...');
});

// ─── Resilient MongoDB connection with infinite retry ─────────────────────────
...
async function connectWithRetry(attempt = 1) {
    try {
        ...
        console.warn(`⚠️  MongoDB not ready (attempt ${attempt}): ${err.message}`);
